-- Function to get transit time distribution for matrix
CREATE OR REPLACE FUNCTION get_transit_time_distribution(
...
BEGIN
  ...
  WITH filtered_events AS (
    ...
    WHERE er.cliente_id = p_cliente_id
      ...
      AND (p_start_date IS NULL OR er.fecha_envio_real >= p_start_date)
      AND (p_end_date IS NULL OR er.fecha_envio_real <= p_end_date)
  ),
  event_counts AS (
    SELECT 
      ciudad_dest,
