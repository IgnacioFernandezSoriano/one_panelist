...
# Encontrar línea de inicio y fin de loadNodeRisks
START=$(grep -n "const loadNodeRisks = async () => {" src/pages/NodosDescubiertos.tsx | cut -d: -f1)
# Encontrar la línea de cierre de la función (buscar el cierre después del try-catch-finally)
END=$(awk -v start="$START" 'NR > start && /^  };$/ {print NR; exit}' src/pages/NodosDescubiertos.tsx)

echo "Function starts at line: $START"
echo "Function ends at line: $END"

# Crear archivo temporal con la nueva función
cat > /tmp/new_loadNodeRisks.txt << 'EOF'
...
head -n $((START - 1)) src/pages/NodosDescubiertos.tsx > /tmp/NodosDescubiertos_new.tsx
cat /tmp/new_loadNodeRisks.txt >> /tmp/NodosDescubiertos_new.tsx
tail -n +$((END + 1)) src/pages/NodosDescubiertos.tsx >> /tmp/NodosDescubiertos_new.tsx

# Reemplazar archivo original
mv /tmp/NodosDescubiertos_new.tsx src/pages/NodosDescubiertos.tsx
