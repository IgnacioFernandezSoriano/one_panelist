      ...
      complete: async (results) => {
        ...
        results.data.forEach((row: any, index: number) => {
          ...
          Object.keys(row).forEach(key => {
            ...
            console.log(`Column mapping: "${key}" -> "${normalizedKey}" = "${row[key]}"`);
          });
          
          console.log('Normalized row:', normalizedRow);
          
          // Buscar clave con diferentes variantes
          const clave = normalizedRow['clave'];
          console.log('Clave found:', clave);
          
          if (!clave || !clave.trim()) {
            console.log('Skipping row - no clave');
            return;
          }
          ...
          languages.forEach((lang: any) => {
            ...
            console.log(`Checking language ${lang.codigo} (${langCode}): "${texto}"`);
        ...
        console.log('Total translations to import:', translations.length);

        if (translations.length === 0) {
          console.error('No translations found!');
          toast({ 
            title: 'Error al importar',
            description: 'No se encontraron traducciones válidas en el archivo. Revisa la consola para más detalles.',
            variant: 'destructive'
          });
          return;
        }
        
        console.log('Attempting to insert translations:', translations.slice(0, 3));
